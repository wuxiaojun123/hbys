...
public class OrderPulishedEvaluateActivity extends BaseActivity implements View.OnClickListener, OrderPulishedEvaluateAdapter.OnAddPhotoListener {
	...
	@BindView(R.id.id_logistics_rating_bar) RatingBar	id_logistics_rating_bar;	// 物流服务

	@BindView(R.id.id_service_rating_bar) RatingBar		id_service_rating_bar;		// 服务态度
	...
	@OnClick({ R.id.iv_title_back, R.id.id_commit_evaluate }) @Override public void onClick(View v) {
		int id = v.getId();
		switch (id) {
	...
	/***
	 * 评价提交 1:内容和评分都写了 2:内容写了，评分没写 3:内容没写，评分写了 4:内容和评分都没写
	 */
	private void commit() {
		...
		PersonalNetwork.getResponseApi().getEvaluateResponse(order_id, evaluate, App.APP_CLIENT_KEY).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
	...
	private void showMyDialog(String msg) {
		new AlertDialog(OrderPulishedEvaluateActivity.this).builder().setTitle(R.string.exit_title).setMsg(msg).setPositiveButton("确定", new View.OnClickListener() {
	...
	@Override public void onItemAddPhotoListener(int position) {
