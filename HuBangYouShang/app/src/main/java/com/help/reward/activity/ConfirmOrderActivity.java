...
public class ConfirmOrderActivity extends BaseActivity implements View.OnClickListener {
	...
	@OnClick({ R.id.iv_title_back, R.id.tv_commit_order }) @Override public void onClick(View v) {
		switch (v.getId()) {
	...
	private void commitOrderRequest() {
		...
		try {
			...
			ShopcartNetwork.getShopcartCookieApi()
					.commitComfirmOrderList(App.APP_CLIENT_KEY, cart_id, if_cart, confirmOrderBean.address_info.address_id, confirmOrderBean.vat_hash, confirmOrderBean.address_api.offpay_hash,
							confirmOrderBean.address_api.offpay_hash_batch, "online", voucher, general_voucher, pay_message)
					.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new BaseSubscriber<CommitOrderResponse>() {
						...
						@Override public void onNext(CommitOrderResponse response) {
							MyProcessDialog.closeDialog();
	...
	private void initData() {
		...
		ShopcartNetwork.getShopcartCookieApi().getComfirmOrderList(App.APP_CLIENT_KEY, cart_id, if_cart, null).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new BaseSubscriber<ConfirmOrderResponse>() {
					...
					@Override public void onNext(ConfirmOrderResponse response) {
						MyProcessDialog.closeDialog();
	...
	@Override protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQ_CODE && resultCode == RES_CODE) {
			...
			if (adapter != null && bean != null) {
				 ...
				 * confirmOrderBean.address_info.address_id = bean.address_id; } else {
				 * confirmOrderBean.address_info = bean; } adapter.setAddressInfo(bean);
