...
public class PwdUpdateActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        switch (id) {
            case R.id.iv_title_back:
                finish();
                ActivitySlideAnim.slideOutAnim(PwdUpdateActivity.this);
    ...
    private void validateCode(final String phoneNumber, String verificationCode) {
        PersonalNetwork.getResponseApi()
                .getModifyPwdStep3(App.APP_CLIENT_KEY, phoneNumber, verificationCode)
                    ...
                    public void onNext(BaseResponse<String> response) {
                        if (response.code == 200) {
                            if (response.data != null) {
                                Intent mIntent = new Intent(PwdUpdateActivity.this, ResetPwdActivity.class);
//                                mIntent.putExtra("phone", phoneNumber);
                                mIntent.putExtra("flag", 2);
                                startActivityForResult(mIntent, REQUEST_CODE);
                                ActivitySlideAnim.slideInAnim(PwdUpdateActivity.this);
                            }
                        } else {
