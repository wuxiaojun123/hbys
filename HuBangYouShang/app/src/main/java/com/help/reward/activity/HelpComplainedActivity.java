...
/**
 * 发布投诉 Created by MXY on 2017/2/19.
 */

public class HelpComplainedActivity extends BaseActivity {

	@BindView(R.id.iv_title_back) ImageView			ivTitleBack;
	...
	@BindView(R.id.iv_release_addphoto) ImageView	ivReleaseAddphoto;
	...
	@OnClick({ R.id.iv_title_back, R.id.tv_title_right, R.id.iv_release_addphoto, R.id.iv_delete1, R.id.iv_delete2, R.id.iv_delete3, R.id.iv_delete4 }) void click(View v) {
		switch (v.getId()) {
	...
	private void subComplainedData(String content) {
		...
		subscribe = HelpNetwork.getHelpApi()
				.getComplainSHPostBean(App.APP_CLIENT_KEY, "complain_shpost", post_id, comment_id, type, content, (String[]) file_names.toArray(new String[file_names.size()]))
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new BaseSubscriber<BaseResponse>() {
					...
					@Override public void onNext(BaseResponse response) {
						MyProcessDialog.closeDialog();
						if (response.code == 200) {
							DialogUtil.showConfirmCancleDialog(HelpComplainedActivity.this, "系统提示", "您的投诉内容已提交\n待被投诉人申诉", "", "确定", new DialogUtil.OnDialogUtilClickListener() {
								...
								@Override public void onClick(boolean isLeft) {
									finish();
