...
public class PropertyAdapter extends BaseAdapter {
    ...
    //用于保存用户的属性集合
    private HashMap<Integer, GoodsInfoBean.PropertyValueInfo> selectProMap = new HashMap<>();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (holder.vgPropContents.getChildCount() == 0 && propertyChildList != null) {
            ...
            for (int j = 0; j < textViews.length; j++) {
                ...
                /*if (j == 0) {
                    LogUtils.e("默认的textview的值是" + textViews[j].getText());
                    textViews[j].performClick();
                }*/
            }
        }
        ...
        public void onClick(View v) {
            ...
            for (int i = 0; i < textViews.length; i++) {
                ...
                if (tv.equals(textViews[i])) {
                    ...
                    // 发送消息，改变库存量和商品图片
                    if (mPropertyOnClickListener != null) {
                        mPropertyOnClickListener.onPropertyClickListener();
                    }
                } else {
                    //其他标签背景变成白色，字体颜色为黑色
                    setNormalTextview(textViews[i]);
    ...
    public PropertyOnClickListener mPropertyOnClickListener;

    public void setPropertyOnClickListener(PropertyOnClickListener listener) {
        this.mPropertyOnClickListener = listener;
    }

    public interface PropertyOnClickListener {
        void onPropertyClickListener();
    }
