...
/**
 * 搜索商品页面结果展示页面 Created by wuxiaojun on 17-3-31.
 */

public class SearchShopResultActivity extends BaseActivity {

	@BindView(R.id.id_drawerlayout) DrawerLayout			mDrawerlayout;

	@BindView(R.id.iv_title_back) ImageView					iv_title_back;

	@BindView(R.id.iv_search_type) TextView					iv_search_type;

	@BindView(R.id.iv_search_line) ImageView				iv_search_line;

	@BindView(R.id.et_search) SearchEditTextView			et_search;

	@BindView(R.id.tv_text) TextView						tv_text;

	@BindView(R.id.layout_alltitle) LinearLayout			layout_alltitle;

	@BindView(R.id.tv_zonghe) TextView						tv_zonghe;

	@BindView(R.id.tv_salenum) TextView						tv_salenum;

	@BindView(R.id.tv_shaixuan) TextView					tv_shaixuan;

	@BindView(R.id.iv_style) ImageView						iv_style;

	@BindView(R.id.id_recycler_view) LRecyclerView			lRecyclerview;

	@BindView(R.id.id_store_recycler_view) LRecyclerView	lStoreRecyclerview;

	@BindView(R.id.tv_reset) TextView						tv_reset;

	@BindView(R.id.tv_ok) TextView							tv_ok;

	@BindView(R.id.myGridView) MyGridView					myGridView;

	@BindView(R.id.tv_freight) TextView						tv_freight;

	@BindView(R.id.tv_cod) TextView							tv_cod;

	@BindView(R.id.tv_refund) TextView						tv_refund;

	@BindView(R.id.tv_protection) TextView					tv_protection;

	@BindView(R.id.tv_quality) TextView						tv_quality;

	@BindView(R.id.tv_sevenDay) TextView					tv_sevenDay;

	@BindView(R.id.et_priceform) EditText					et_priceform;

	@BindView(R.id.et_priceto) EditText						et_priceto;
	...
	@OnClick({ R.id.iv_title_back, R.id.iv_search_type, R.id.ll_zonghe, R.id.tv_salenum, R.id.ll_shaixuan, R.id.ll_style, R.id.tv_ok, R.id.tv_reset, R.id.iv_showAll, R.id.tv_freight, R.id.tv_cod,
			R.id.tv_refund, R.id.tv_protection, R.id.tv_quality, R.id.tv_sevenDay }) void click(View v) {
		switch (v.getId()) {
			...
			case R.id.iv_search_type:
				new GoodsTypeSearchPop().showPopupWindow(this, iv_search_type).setOnTypeChooseListener(new GoodsTypeSearchPop.OnTypeChooseListener() {
					...
					@Override public void onType(String type) {
						searchType = type;
	...
	private void requestData(boolean isFirst) {
		...
		subscribe = ShopMallNetwork.getShopMallMainApi()
				.getSearchGoodsResponse("goods", "goods_list", curpage, key, order, price_from, price_to, b_id, (String[]) service.toArray(new String[service.size()]), keyword, gc_id, store_id)
				.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new BaseSubscriber<StoreDetailAllResponse>() {
					...
					@Override public void onNext(StoreDetailAllResponse response) {
						lRecyclerview.refreshComplete(15);
						MyProcessDialog.closeDialog();
	...
	private void requestStoreData(boolean isFirst) {
		...
		subscribe = ShopMallNetwork.getShopMallMainApi().getSearchStoreResponse("store", "listStore", curpage, keyword).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
				.subscribe(new BaseSubscriber<SearchStoreResponse>() {
					...
					@Override public void onNext(SearchStoreResponse response) {
						lStoreRecyclerview.refreshComplete(15);
						MyProcessDialog.closeDialog();
						if (response.code == 200) {
							if (response.data != null) {
	...
	private void initBrandData() {
		subscribe = ShopMallNetwork.getShopMallMainApi().getBrandBeanResponse(gc_id).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new BaseSubscriber<BrandResponse>() {
			...
			@Override public void onNext(BrandResponse response) {
				if (response.code == 200) {
					if (response.data != null) {
