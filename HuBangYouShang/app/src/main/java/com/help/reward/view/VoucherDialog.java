...
/**
 * 可用优惠卷 Created by wuxiaojun on 17-7-7.
 */
...
public class VoucherDialog implements VoucherAdapter.OnVoucherCheckedChangedListener {
	...
	private void initListener() {
		mAdapter = new VoucherAdapter(mContext, this, mList);
		...
		btn_close.setOnClickListener(new View.OnClickListener() {
			...
			@Override public void onClick(View v) {
				if (id_cb_use_voucher.isChecked()) {
					...
					for (VoucherBean voucherBean : mList) {
						if (voucherBean.isChecked) {
							...
							// voucherBean.voucher_store_id + "--" + voucherBean.voucher_price);
				...
				if (mOnConfirmClickListener != null) {
					mOnConfirmClickListener.onConfirmClick(mVoucherMap);
				}
				dismissDialog();
			}
		});
	...
	@Override public void onCheckedChangedListener(String voucherId, boolean userVoucher) {
	...
	public void notifyDataSetChanged() {
		if (mLRecyclerView.getScrollState() == RecyclerView.SCROLL_STATE_IDLE || !mLRecyclerView.isComputingLayout()) {
			mAdapter.notifyDataSetChanged();
		}
	}

	/****
	 * 监听recyclerview的滑动 有两种type ,type1显示一种ui type2显示另外一种ui
	...
	public void setOnConfirmClickListener(OnConfirmClickListener clickListener) {
		this.mOnConfirmClickListener = clickListener;
	}

	public interface OnConfirmClickListener {

		void onConfirmClick(HashMap<String, String> mVoucherMap);
	}
