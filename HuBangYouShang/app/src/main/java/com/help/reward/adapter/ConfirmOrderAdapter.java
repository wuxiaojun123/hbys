...
public class ConfirmOrderAdapter extends RecyclerView.Adapter<SuperViewHolder> {
	...
	private HashMap<String, String>							mVoucherMap			= new HashMap<>();										// 优惠券
	...
	@Override public SuperViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
	...
	@Override public void onBindViewHolder(SuperViewHolder holder, int position) {
	 ...
	 * @param bean
	 */
	private void bindVoucherView(final TextView tv_coupon, final ConfirmOrderResponse.ConfirmCartList bean) {
		VoucherBean defaultVoucherBean = bean.store_voucher_info;

		if (defaultVoucherBean == null && mVoucherMap.isEmpty()) {
			...
			if (mVoucherMap.isEmpty()) {
				mVoucherMap.put(defaultVoucherBean.voucher_id, defaultVoucherBean.voucher_t_id + "|" + defaultVoucherBean.voucher_store_id + "|" + defaultVoucherBean.voucher_price + ",");
			}
			...
			dialog.setOnConfirmClickListener(new VoucherDialog.OnConfirmClickListener() {

				@Override public void onConfirmClick(HashMap<String, String> voucherMap) {
					mVoucherMap = voucherMap;
					if (voucherMap == null || voucherMap.isEmpty()) { // 不适用优惠券,更新商品总价
						tv_coupon.setText("不使用优惠劵");
					} else {
						tv_coupon.setText("可用优惠劵");
					}
					refreshInitData();
	...
	private void doBottomView(SuperViewHolder holder) {
		...
		final ToggleButton btn_switch = (ToggleButton) holder.itemView.findViewById(R.id.btn_switch); // 开关按钮
		// 有可能没有通用劵的情况，应该进行区分
		if (TextUtils.isEmpty(available_general_voucher) || "".equals(available_general_voucher) || "0".equals(available_general_voucher)) {
			...
			btn_switch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
				...
				@Override public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
					...
					checkedChangedUseGeneralVoucher(btn_switch);
	...
	private void checkedChangedUseGeneralVoucher(ToggleButton btn_switch) {
	...
	private void refreshInitData() {
		((ConfirmOrderActivity) mContext).initData(false);
	}
